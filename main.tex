...
The practical challenges related to the distribution of large MD simulation trajectories are solved in the NMRlipids databank using the overlay structure. 
...
Quality of membrane simulations with different force fields have been evaluated against experimental data during parameterization and in separate comparison studies~\cite{botan15,ollila16,catte16,pluhackova16,perez17,leonard19}, but universal quality measure for membrane simulations is not defined and controversial results are often reported from simulations~\cite{antila22b}. The ambiguity in defining the quality of simulations complicates the selection of proper simulation parameters and estimation of the reliability of simulation results, thereby being a major obstacle in many applications of membrane MD simulations. The NMRlipids
databank enables rapid quality evaluation of MD simulations using the quantitative quality measure for membrane simulations. The conformational ensembles of individual lipid molecules are evaluated against C-H bond order parameters from NMR experiments, which is complement by x-ray scattering form factors arising from overall membrane dimensions via electron density profile. Both of these are robust experimental measurables that can be directly connected to the simulation data~\cite{ollila16}. 
...
The top quality simulations based on average order parameter quality are shown in Fig.~\ref{fig:quality} A and the direct comparison between the best simulation (POPC with OPLS3e) and experiments in shown in Fig.~\ref{fig:quality} D. This model reproduces the experimental data very well with the exception of third carbon in the $\it{sn}$-1 chain and carbons 8 and 9 close to double bond in $\it{sn}$-2 chain.
%Full details of the NMRlipids quality measure for lipid bilayer simulations are given in the methods section and supplementary information.
The top ranking simulation for the order parameter quality of POPE lipid are shown in Fig.~\ref{fig:quality} B. In this case the best overall quality is found in GROMOS-CKP simulation (direct comparison shown in Fig.~\ref{fig:quality} E, but clear discrepancies with experiments are present in this model for glycerol backbone and $\it{sn}$-2 chain. On the other hand, glycerol backbone and headgroup for POPE is best described in CHARMM36 model (direct comparison shown in Fig.~\ref{fig:quality} F), but its overall quality suffers from overestimated order in acyl chains. Scatter plots and Pearson correlation coefficients in Fig.~\ref{fig:quality} C reveal strong correlation between membrane area per lipid and thickness. The area per lipid is also strongly correlated with the average order of $\it{sn}$-1 chain, suggesting that its order parameters can used as a proxy for the area per lipid in addition to the conformational ensemble. On the other hand, the first form factor minima is correlated stronger with membrane thickness (Fig.~\ref{fig:quality} C) than with the area per lipid (Fig.~\ref{fig:QualityCorrelationsSI}). 
...
%averaging Here we demonstrate the power of the NMRlipids databank by analysing how 
...
%from the carbon and hydrogen positions using the 
%definition
...
%by estimating
%the average probability of C-H bond order parameters to agree with the values from NMR experiments. 
...
% shifted to $\bar{X}$ and scaled with $s$. 
