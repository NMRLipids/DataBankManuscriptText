...
Quality of conformational ensembles of lipid molecules in simulations are evaluated by estimating
...
and conformational ensembles of individual lipids can be assumed to be independent, the random variable
...
and $n$ is the sample size (number of lipids),
has a Student's t-distribution with $n-1$ degrees of freedom and the mean of $\mu$.
Because lipid bilayer simulations contain at least dozens of lipids,
the Student's t-distribution could be safely approximated with a normal distribution in here.
However, normal distribution gives values below the numerical accuracy of computers when values
are far away from the mean which leads to problems when evaluating simulations with poor performance.
To avoid such numerical instability, we use the first order Student's t-distribution for the equation \ref{studentT}.
...
The quality of separate fragments in each lipid type within a simulation
are then estimated by averaging the propabilities for individual C-H bond order parameters
to locate within experimental errors
%qualities over  belonging to that fragment
and dividing this with the percentage of order parameters for which the quality is available within the fragment, $p$
\begin{equation}
