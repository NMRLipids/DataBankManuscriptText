...
\ref{DatabankStructure}
\ref{fig:permeationSI}


\subsection{Molecule and atom naming convention} \label{naming}
When analysing simulations, atoms and molecules needs to be often called by their names defined in the simulation trajectory. However, these names often vary between the force fields because the universal naming convention has not been defined for lipids. To enable automatic analyses over simulations with different atom and molecule names in the NMRlipids databank, we have defined unique naming conventions for molecules and atoms therein. Unique abbreviations used in the NMRlipids databank for each molecule are listed in table~\ref{tab:abbreviations} in the supplementary information. Atom names in a simulation are connected to unique atom names defined for each molecule using mapping files located at {\it /Scripts/BuildDatabank/mapping\_files} in the NMRlipids repository. 
%Mapping files are in yaml format where unique atom names are connected to force field specific names. 
Also residue names for each atom and whether they belong to headgroup, glycerol bakcbone, or acyl chain region are defined in the mapping files. For more details about mapping files, see~\url{https://nmrlipids.blogspot.com/2022/04/new-yaml-format-of-mapping-files.html}). In practise, 
%information about molecule and atom names are given, where 
mapping file names and force field specific molecule names corresponding the unique abbreviation (table~\ref{tab:abbreviations}) are given in the COMPOSITION dictionary in README.yaml files. 
...
Anyone can add a new simulation into the NMRlipids databank by first creating an info.yaml file containing the manually entered information listed in table~\ref{tab:READMEkeys} and then adding this file to {\it /Scripts/BuildDatabank/info\_files} folder in the NMRlipids databank repository via pull requests. Pull requests are manually accepted and automatically extracted data for README.yaml files,  listed in table~\ref{tab:READMEkeys}, will be calculated using the {\it /Scripts/BuildDatabank/AddData.py} script. Current content of the NMRlipids databank is composed of the simulations found from Zenodo repository with an appropriate licence. Most, but not all, of these trajectories originate from previous NMRlipids projects~\cite{botan15,catte16,antila19,bacle21}.
...
Flip-flop rates were calculated using {\it AssignLeaflets} and {\it FlipFlop} tools from LiPyphilic package~\cite{LiPyphilic2021}. Headgroup atoms of each molecule defined in the mapping file were used to determine in which leaflet the molecule locates. The midplane cut-off defining the region between leaflets was 1\,nm and frame cut-off was 100. This means that if the headgroup of a molecule entered within the distance of 1\,nm from the bilayer midplane and was found from the opposing leaflet after 100 steps, this event was considered as a successfull flip-flop event. A code that finds flip-flop rates from all simulations in the databank is available at {\it scripts/FlipFlop.py} and the resulting flip-flop events at {\it Data/Flipflops/} in the repository at~\url{https://github.com/NMRLipids/DataBankManuscript/}. 
